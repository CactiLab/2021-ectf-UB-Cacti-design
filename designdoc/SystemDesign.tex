...
\begin{itemize}
  % \item[Step1.]
  \item[Step1. ] When adding SED$_k$ to the deployment, SSS generates a pair of keys ($pk_k$, $sk_k$) for it.
  Stores as \verb|SCEWL_ID|.$pub$, \verb|SCEWL_ID|.$pri$
  \item[Step2. ] When building controller for SED$_k$, the key pair ($pk_k$, $sk_k$) will be copied to this controller container.
