...
SSS stores the keys in files as \verb|SCEWL_ID|.$pub$, \verb|SCEWL_ID|.$pri$ at its local filesystem.
  ...
  \item[Step 3.] At registration stage, the SED$_k$ uses its own private key ($sk_k$) to encrypt the registration message and send to the SSS.
...
%   \end{itemize}
  ...
  \item Each SED maintains a table containing other SED's sequence numbers with initial value of zero.
...
\end{tabular}
\end{center}
  \item Suppose SED$_a$ communicates with the SED $b$, $a$ sends its corresponding sequence of b($0$) in the message. After decrypting the message SED $b$ checks with the SED $a$ value if it's less or equal to the received number then it's a legit message. 
