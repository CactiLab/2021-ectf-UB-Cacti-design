...
int main(void) {
  ...
  while (data[0] != 'q' || data[1] != 'u' ||
    ...
    // print message
    fprintf(log, "***************************\nserver ID: %d\n", SCEWL_ID);
    fprintf(log, "echo_server1: Waiting for message...\n");

    // receive message
    len = scewl_recv(data, &src_id, &tgt_id, DLEN, 1);

    if (tgt_id == SCEWL_BRDCST_ID) {
      fprintf(log, "echo_server1: Received broadcast\n");
    } else {
      fprintf(log, "echo_server1: Received direct message\n");
    }
  ...
  // degister
  fprintf(log, "echo_server1: Deregistering...\n");
  if (scewl_deregister() != SCEWL_OK) {
    fprintf(log, "echo_server1: BAD DEREGISTRATION!\n");
  }
  fprintf(log, "echo_server1: Exiting...\n");
}
