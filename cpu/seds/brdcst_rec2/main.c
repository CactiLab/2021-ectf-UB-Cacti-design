...
int main(void) {
  ...
  if (scewl_register() != SCEWL_OK) {
    fprintf(log, "client112: BAD REGISTRATION! Reregistering...\n");
    if (scewl_deregister() != SCEWL_OK) {
      fprintf(log, "client112: BAD DEREGISTRATION!\n");
      return 1;
    }
    if (scewl_register() != SCEWL_OK) {
      fprintf(log, "client112: BAD REGISTRATION! CANNOT RECOVER\n");
      return 1;
    }
  }

  // fprintf(log, "client112: Sending hello...\n");
  // scewl_send(TGT_ID, 13, msg);

  // receive response (block until response received)
  fprintf(log, "client112: Waiting for response...\n");
  scewl_recv(data, &src_id, &tgt_id, BUF_SZ, 1);

  // check if response matches
  if (!strcmp(msg, data)) {
    // decode and print flag
    fprintf(log, "client112: OK!\n");
  } else {
    fprintf(log, "client112: BAD!\n");
  }

  // deregister
  fprintf(log, "client112: Deregistering...\n");
  if (scewl_deregister() != SCEWL_OK) {
    fprintf(log, "client112: BAD DEREGISTRATION!\n");
  }
  fprintf(log, "client112: Exiting...\n");
}
